...
const TwoPersonEditEvent = ({ event }) => {
  ...
  console.log(fromDate, toDate, eventDuration)
  useEffect(() => {
    // console.log('event inside ue', event)
    setFromDate(event.fromDate.split('T')[0])
    setToDate(event.toDate.split('T')[0])
    setEventDuration(event.eventDuration)
    setLocation(event.meetingLocation)
    // setId(event.id)
  },[event])
...
const getDayOfWeek = (date) => {
 ...
 const editData = async(editedEventInfo)=>{
  const id = event._id
  console.log(id)
  try {
    const res = await axios.put(`/api/createEvent/${id}`, editedEventInfo)
    return res;
  } catch (error) {
    console.log(error)
    throw error;
  ...
  // form Handler to post edited data
  const formHandler = (e) => {
    e.preventDefault()
    const form = e.target;
  ...
  <div className="text-sm space-y-3 my-3">
          {fromDate && toDate
            ? getDatesInRange(fromDate, toDate).map((date, index) => {
                // Find the corresponding time slot for the current date
                const selectedTimeSlot = event?.dateAndTimeArray.find(
...
{/* meeting link */}
          <div className="space-y-5">
            <div className="">
              ...
              <InputField
               className="w-[380px]  h-[40px] rounded-md text-gray-700 bg-white border focus:border-purple-400 dark:focus:border-purple-300 focus:outline-none focus:ring focus:ring-opacity-40 focus:ring-purple-300 p-2"
               type="text"
               name="meetingLink"
...
export default TwoPersonEditEvent;
